...
class Appt:
    ...
    def __lt__(self, other: "Appt"):
        return self.finish <= other.start

    def __gt__(self, other: "Appt"):
        return self.start >= other.finish

    def overlaps(self, other: "Appt") -> bool:
        """Is there a non-zero overlap betwee n these periods"""
...
class Agenda:
    ...
    def conflicts(self) -> "Agenda":
        ...
        self.sort()
        conflicts = Agenda()
        for i, appt in enumerate(self.elements):
            for j in range(i+1, len(self.elements)):
                if appt.overlaps(self.elements[j]):
                    conflicts.append(appt.intersect(self.elements[j]))
                else:
                    break
        return conflicts
