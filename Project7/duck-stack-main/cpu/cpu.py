...
class ALU(object):
    ...
    def exec(self, op: OpCode, in1: int, in2: int) -> tuple[int, CondFlag]:
        try:
            val = ALU.ALU_OPS[op](in1, in2)
        except ZeroDivisionError:
            return (0, CondFlag.V)
        if val == 0:
            return (val, CondFlag.Z)
        elif val < 0:
            return (val, CondFlag.M)
        else:
            return (val, CondFlag.P)
